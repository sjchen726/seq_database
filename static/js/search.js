...
function searchTable(filters = {}) {
    ...
    rows.forEach(row => {
        ...
        const match = Object.entries(filters).every(([key, value]) => {
            ...
            if (key === 'modifySeq') {
                // 对 'modifySeq' 字段进行特殊处理
                const modifiedValue = queryList.map(v => v.replace(/[ofs\s]/g, '')); // 同样移除 'o' 和 's'
                return modifiedValue.some(val => modifySeqData.includes(val));
            }
