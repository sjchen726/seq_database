    ...
    $('body').on('click', '.clone-seq-btn', function(e) {
        ...
        $.get('/clone_delivery/', { strand_id: strand }, function(resp) {
            ...
            rows.forEach(function(r, idx) {
                var html = '';
                html += '<div class="clone-row p-2 border mb-2">';
                ...
                html += '<div class="form-group col-md-4"><label class="text-left">Project</label><input name="Project" class="form-control" value="' + (r.Project || '') + '" readonly /></div>';
                html += '<div class="form-group col-md-4"><label class="text-left">Target</label><input name="Target" class="form-control" value="' + (r.Target || '') + '" readonly /></div>';
                html += '<div class="form-group col-md-4"><label class="text-left">Seq_type</label><input name="Seq_type" class="form-control" value="' + (r.Seq_type || '') + '" readonly /></div>';
                html += '</div>';
                // second row: Modify_seq occupies full width
                html += '<div class="form-row">';
                html += '<div class="form-group col-12"><label class="text-left">Modify_seq</label><input name="Modify_seq" class="form-control" value="' + (r.Modify_seq || '') + '" /></div>';
                html += '</div>';
                // third row: delivery5 and delivery3 share the row
                html += '<div class="form-row">';
                html += '<div class="form-group col-md-6"><label class="text-left">delivery5</label><input name="delivery5" class="form-control" value="' + (r.delivery5 || '') + '" /></div>';
                html += '<div class="form-group col-md-6"><label class="text-left">delivery3</label><input name="delivery3" class="form-control" value="' + (r.delivery3 || '') + '" /></div>';
                html += '</div>';
                // fourth row: Strand_MWs, Parents, Remark share a single row (3 columns)
                html += '<div class="form-row">';
                html += '<div class="form-group col-md-4"><label class="text-left">Strand_MWs</label><input name="Strand_MWs" class="form-control" value="' + (r.Strand_MWs || '') + '" /></div>';
                html += '<div class="form-group col-md-4"><label class="text-left">Parents</label><input name="Parents" class="form-control" value="' + (r.Parents || '') + '" /></div>';
                html += '<div class="form-group col-md-4"><label class="text-left">Remark</label><input name="Remark" class="form-control" value="' + (r.Remark || '') + '" /></div>';
                html += '</div>';
                html += '</div>';
